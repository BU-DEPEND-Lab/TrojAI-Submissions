...
def analyze(param,nbins=100,szcap=4096):
    ...
    if len(param.shape)==4:
        ...
        fv=torch.cat((e_mean.cpu(), e_std.cpu(), e_norm.cpu(), top_k_e.cpu(), bot_k_e.cpu(), s_mean.cpu(), s_std.cpu(), s_norm.cpu(), e2_hist,er_hist,ec_hist,eig_persist,w_hist,wabs_hist),dim=0);
        return [fv];
    else:
        return [];
