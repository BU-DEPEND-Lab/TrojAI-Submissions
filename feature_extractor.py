    ...
    def manual_configure(self, models_dirpath: str):
        ...
        for _ in range(len(flat_models)):
            ...
            for i, model in enumerate(models):
                ...
                model_feats = np.hstack(list(models[i].values())[-1: -2: -1])[-1:0:-1]#[::int(model_feats.shape[-1] / self.ICA_features)]
                clean_grad_feats = np.hstack(list(grads[i].values())[-1: -2: -1])[-1:0:-1]#[::int(clean_grad_feats.shape[-1] / self.ICA_features)]
