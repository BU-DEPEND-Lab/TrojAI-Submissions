...
class FeatureExtractor(object):
    ...
    def manual_configure(self, models_dirpath: str):
        ...
        for _ in range(len(flat_models)):
            ...
            for i, model in enumerate(models):
                ...
                model_feats = np.hstack(list(models[i].values())[len(models[i])-1:0:-1])[-1:0:-1]#[::int(model_feats.shape[-1] / self.ICA_features)]
                clean_grad_feats = np.hstack(list(grads[i].values())[len(grads[i])-1:0:-1])[-1:0:-1]#[::int(clean_grad_feats.shape[-1] / self.ICA_features)]
    ...
    def infer_one_model(self, model_filepath):
        ...
        feats = np.hstack((model_feats, clean_grad_feats)) #, poisoned_grad_feats)).tolist()
        feats = np.pad(feats, [(0, 0), (0, 2 * self.ICA_features - feats.shape[-1])], mode='constant')
        model_feats = np.hstack(list(flat_model.values())[len(flat_model)-1:0:-1])[-1:0:-1]#[::int(model_feats.shape[-1] / self.ICA_features)]
        clean_grad_feats = np.hstack(list(flat_grad.values())[len(flat_grad)-1:0:-1])[-1:0:-1]#[::int(clean_grad_feats.shape[-1] / self.ICA_features)]
        feats = [model_feats.tolist()[:self.ICA_features] + clean_grad_feats.tolist()[:self.ICA_features]]
        assert np.asarray(feats).shape[-1] == 2 * self.ICA_features
