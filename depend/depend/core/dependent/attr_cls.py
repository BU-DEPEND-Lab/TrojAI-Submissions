...
from depend.models.cls import FCModel, CNNModel
...
class AttributionClassifier(Dependent):
    ...
    def get_attribute_from_image(self, model, obs):
        ...
        return {k: torch.mul(norm(v.grad.data), v) if k == 'image' else v for k, v in obs.items()}
    ...
    def infer(self, model) -> List[float]:
        ...
        #logger.info(obs['direction'].grad)
        norm = lambda attrs: (attrs  - attrs.min()) / (attrs.max() - attrs.min() + 1.e-5)
        inputs = {k: torch.mul(norm(v.grad.data), v) if k == 'image' else v for k, v in exps.items()}
        pred = (cls(inputs) * np.random.random()).mean().item() 
