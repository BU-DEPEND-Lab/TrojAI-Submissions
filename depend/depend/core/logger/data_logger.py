...
class DataLogger(object):
    ...
    automatically a log directory, save numpy data array and the current dependent.
    ...
    def log_dependent(
        ...
        Args:
            dependent (Dependent): The dependent to be saved;
            epoch (int, None): optional epoch number to
                be added to the dependent file currently saved;
            full_save (bool, False): whether to save the full
                data from the dependent or not.

        """
        epoch_suffix = '' if epoch is None else '-' + str(epoch)

        filename = 'dependent' + self._suffix + epoch_suffix + '.msh'
        path = self._results_dir / filename
        dependent.save(path, full_save=full_save)

    def log_best_dependent(self, dependent, J, full_save=False):
        """
        Log the best dependent so far into the log folder. The dependent
        is logged only if the current performance is better
        than the performance of the previously stored dependent.

        Args:
            dependent (Dependent): The dependent to be saved;
            J (float): The performance metric of the current dependent;
            full_save (bool, False): whether to save the full
                data from the dependent or not.
        ...
        if J >= self._best_J:
            ...
            filename = 'dependent' + self._suffix + '-best.msh'
            path = self._results_dir / filename
            dependent.save(path, full_save=full_save)
